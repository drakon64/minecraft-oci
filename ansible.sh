	...
	case ${arg} in
		i)
			...
			if [ "$3" = "bedrock" ] ; then
				...
				echo "$2 edition=java command='/usr/bin/java -Xmx1024M -Xms1024M -jar server.jar nogui' new_server=false restore_backup=false" > ansible/inventory
			fi
			;;
		n)
			check_ip "$@"
			...
			if [ "$3" = "bedrock" ] ; then
				...
				echo "$2 edition=java command='/usr/bin/java -Xmx1024M -Xms1024M -jar server.jar nogui' new_server=true restore_backup=false" > ansible/inventory
			fi
			;;
		b)
			check_ip "$@"
