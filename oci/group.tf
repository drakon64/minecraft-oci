...
resource "oci_identity_policy" "minecraft_backup" {
	...
	statements = [
		...
		"Allow dynamic-group ${oci_identity_dynamic_group.minecraft_backup.name} to manage objects in compartment id ${oci_identity_compartment.tf-compartment.id} where all {target.bucket.name='${oci_objectstorage_bucket.minecraft_backup.name}', any {request.permission='OBJECT_INSPECT', request.permission='OBJECT_READ', request.permission='OBJECT_CREATE'}}"
	]
}
